...
%1 has some fixed triangles assigned first, 2 has all triangles generated randomly
...
figureSubfolder=[figurefolder,'\ScanSecondOrderR2_Density',num2str(Density)];
mkdir(figureSubfolder);
%% Start multiple numerical runs
for realization=1%2:10%:10% the number of numerical replicates

filename=[figureSubfolder,'\Rewire_',num2str(realization),'_InitialNet_',num2str(InitialNet),'.mat'];%Scan has target 1.8/1.4, Scan2 has target value 1

tic;
for NumberNode=[6,10:5:100]%[6:10]% Scan different network sizes
    
    TriangleNum=NumberNode*Density;%NumberNode*2;%Fix the number of triangles
...
while TriangleNumCurrent<min(TriangleNum,nchoosek(NumberNode,3))
...
if InitialNet==2% Second type of initialized network
 %Add randomly to have TriangleNum new triangles
TriangleNumCurrent=0;
TriangleNodes=1:NumberNode;
while TriangleNumCurrent<min(TriangleNum,nchoosek(NumberNode,3)) || length(TriangleNodes)<NumberNode
    id=randperm(NumberNode,3);
     P = perms(id); %symmetric tensor for each triangle
    for k=1:size(P,1)
    AdjacencyTensor(P(k,1),P(k,2),P(k,3))=1;
    ...
    [AdjConverted,triangle_initial]=ConvertTriangleToAdjacency(AdjacencyTensor);
    TriangleNumCurrent=size(triangle_initial,1);
    TriangleNodes=unique(triangle_initial);
end
end
